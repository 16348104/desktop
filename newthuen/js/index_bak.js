 ...
 var LIMITDATE="2014-03-03";//看能到的最早年月，测试时用2月，上线时请用3月，因为3月之前没有这个版本的图片
 var PRELOADIMGNUM=5;//预加载图片个数
$(function(){
$.ajax({url:'/application/gettime.jsp',async:false, success: function(data){
				...
				var date=takefulldate(y,m,d);
				 if(jdata[date]){
					 $("#calendarDay a").each(function(){
						 if($(this).text()==d) {$(this).addClass("current");}else{$(this).removeClass("current");}
						})
...
function parseISO8601(dateStringInRange) {  
 ...
 function Calendar(data) {
	 ...
	 var MonthInEn=["Jan.","Feb.","Mar.","Apr.","May.","Jun.","Jul.","Aug.","Sep.","Oct.","Nov.","Dec."];
	...
	this.cal=function(){	 
		  ...
		  for (var intLoop = 1; intLoop <= endDay;intLoop++) 
		  {
			...
			$("#calendarMonthEn").html(MonthInEn[month-1]);
